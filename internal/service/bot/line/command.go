...
func (c *LineCommandHandler) CommandStart(userID string) error {
	text := `台股機器人指令指南🤖
...
// 處理 /p 命令 - 股票績效圖表 (折線圖)
func (c *LineCommandHandler) CommandPerformanceChart(userID, symbol string) error {
	if symbol == "" {
...
func (c *LineCommandHandler) CommandTodayStockPrice(userID, symbol, date string) error {
	// 輸入驗證
	if symbol == "" {
...
// 處理 /k 命令 - 歷史K線圖
func (c *LineCommandHandler) CommandHistoricalCandles(userID, symbol string) error {
	if symbol == "" {
		return c.sendMessage(userID, "請輸入股票代號")
	}
...
// 處理 /n 命令 - 股票新聞
func (c *LineCommandHandler) CommandNews(userID, symbol string) error {
	if symbol == "" {
		return c.sendMessage(userID, "請輸入股票代號")
	}
	...
	return c.sendMessageWithButtons(userID, newsMessage.Text, newsMessage.Buttons)
}

// 處理 /m 命令 - 大盤資訊
func (c *LineCommandHandler) CommandDailyMarketInfo(userID string, count int) error {
	// 呼叫業務邏輯
	...
	// 發送回應
	return c.sendMessage(userID, messageText)
}

// 處理 /t 命令 - 交易量前20名
func (c *LineCommandHandler) CommandTopVolumeItems(userID string) error {
	// 取得交易量前20名資料
...
// 處理 /i 命令 - 股票資訊（可指定日期）
func (c *LineCommandHandler) CommandStockInfo(userID, symbol, date string) error {
	if symbol == "" {
		return c.sendMessage(userID, "請輸入股票代號")
	}
...
// 處理 /r 命令 - 股票財報
func (c *LineCommandHandler) CommandRevenue(userID, symbol string) error {
	if symbol == "" {
		return c.sendMessage(userID, "請輸入股票代碼")
	}
...
// 處理 /sub 命令 - 訂閱功能
func (c *LineCommandHandler) CommandSubscribe(userID, item string) error {
	return c.updateUserSubscription(userID, item, "active")
}

// 處理 /unsub 命令 - 取消訂閱功能
func (c *LineCommandHandler) CommandUnsubscribe(userID, item string) error {
	return c.updateUserSubscription(userID, item, "inactive")
}

// updateUserSubscription 更新使用者訂閱狀態
func (c *LineCommandHandler) updateUserSubscription(userID, item, status string) error {
...
// 處理 /add 命令 - 新增股票訂閱
func (c *LineCommandHandler) CommandAddStock(userID, symbol string) error {
	if symbol == "" {
		return c.sendMessage(userID, "請輸入股票代號")
	}
...
// 處理 /del 命令 - 刪除股票訂閱
func (c *LineCommandHandler) CommandDeleteStock(userID, symbol string) error {
	if symbol == "" {
		return c.sendMessage(userID, "請輸入股票代號")
	}
...
// 處理 /list 命令 - 列出訂閱項目
func (c *LineCommandHandler) CommandListSubscriptions(userID string) error {
	// 取得使用者資料
	user, err := c.userService.GetUserByAccountID(userID, models.UserTypeLine)
	if err != nil {
		logger.Log.Error("取得使用者失敗", zap.Error(err))
...
// 發送訊息
func (c *LineCommandHandler) sendMessage(userID, text string) error {
	_, err := c.botClient.PushMessage(userID, linebot.NewTextMessage(text)).Do()
	if err != nil {
...
// 發送帶有按鈕的訊息
func (c *LineCommandHandler) sendMessageWithButtons(userID, text string, buttons []linebot.TemplateAction) error {
	if len(buttons) == 0 {
		return c.sendMessage(userID, text)
	}
	...
	_, err := c.botClient.PushMessage(userID, linebot.NewTemplateMessage("按鈕", template)).Do()
	if err != nil {
...
// 發送圖片
func (c *LineCommandHandler) sendPhoto(userID string, data []byte, caption string) error {
